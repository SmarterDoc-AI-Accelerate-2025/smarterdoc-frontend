    ...
    <main className="relative min-h-screen flex flex-col items-center px-6 py-10">
      ...
      <section className="backdrop-blur-md bg-white/60 rounded-3xl shadow-lg p-6 w-full max-w-6xl z-10 mb-10">
        <h2 className="text-xl font-semibold text-[#433C50] mb-4">
          Your {doctors.length} specially <span className="text-[#5F72BE]">AI-recommended</span> doctors
        </h2>

        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            {doctors.map((doc, index) => (
              <div key={index} className="flex items-center bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                <Image src={doc.img || "/doctor.png"} alt={doc.name} width={80} height={80} className="rounded-full mr-4" />
                <div className="flex-1">
                  ...
                  <div className="flex items-center text-yellow-500 text-sm my-1">
                    ...
                    <span className="ml-1 text-gray-700">{doc.rating} ({doc.reviews} reviews)</span>
                  </div>
                  <p className="text-sm text-gray-600">{doc.address}</p>
                  <p className="text-sm text-gray-500">Insurance: {doc.insurance_accepted?.join(", ")}</p>
                </div>
